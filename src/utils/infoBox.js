...
export default class InfoBox extends Phaser.GameObjects.Container{
    ...
    constructor(config){
        ...
        this.setInteractive();
        this.add(this.scene.add.text(-this.width/2+10,-this.height/2,this.entry.name,TEXT_CONFIG.SubHeading).setTint(PALETTE_HEX.DarkerGrey).setOrigin(0.0));
        this.add(this.scene.add.text(-this.width/2+10,-this.height/2+36,config.expanded ? this.entry.long_desc : this.entry.description,TEXT_CONFIG.Paragraph).setTint(PALETTE_HEX.DarkerGrey).setOrigin(0.0));
